...
export default class Detector {
  ...
  processFinderPatternInfo(info) {
    ...
    if (provisionalVersion.getAlignmentPatternCenters().length > 0) {
      ...
      const estAlignmentX = /*(int) */ Math.floor(
        topLeft.getX() + correctionToTopLeft * (bottomRightX - topLeft.getX()),
      )
      const estAlignmentY = /*(int) */ Math.floor(
        topLeft.getY() + correctionToTopLeft * (bottomRightY - topLeft.getY()),
      )
  ...
  calculateModuleSizeOneWay(pattern, otherPattern) {
    const moduleSizeEst1 /*float*/ = this.sizeOfBlackWhiteBlackRunBothWays(
      /*(int) */ Math.floor(pattern.getX()),
      /*(int) */ Math.floor(pattern.getY()),
      /*(int) */ Math.floor(otherPattern.getX()),
      /*(int) */ Math.floor(otherPattern.getY()),
    )
    const moduleSizeEst2 /*float*/ = this.sizeOfBlackWhiteBlackRunBothWays(
      /*(int) */ Math.floor(otherPattern.getX()),
      /*(int) */ Math.floor(otherPattern.getY()),
      /*(int) */ Math.floor(pattern.getX()),
      /*(int) */ Math.floor(pattern.getY()),
