...
export default class FinderPatternFinder {
  ...
  public find(hints: Map<DecodeHintType, any>): FinderPatternInfo /*throws NotFoundException */ {
    const tryHarder: boolean =
      hints !== null && hints !== undefined && undefined !== hints.get(DecodeHintType.TRY_HARDER)
    const pureBarcode: boolean =
      hints !== null && hints !== undefined && undefined !== hints.get(DecodeHintType.PURE_BARCODE)
  ...
  protected handlePossibleCenter(
    ...
    let centerI: number /*float*/ = this.crossCheckVertical(
      i,
