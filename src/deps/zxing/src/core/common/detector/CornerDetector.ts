...
export default class CornerDetector {
  ...
  private findCorners(right: number, left: number, down: number, up: number): ResultPoint[] {
    ...
    const topRightCornerCenter = new ResultPoint(
      topRightCorner.getX() - xCorrection,
      topRightCorner.getY() + yCorrection,
    )
    const bottomRightCornerCenter = new ResultPoint(
      bottomRightCorner.getX() - xCorrection,
      bottomRightCorner.getY() - yCorrection,
    )
    const bottomLeftCornerCenter = new ResultPoint(
      bottomLeftCorner.getX() + xCorrection,
      bottomLeftCorner.getY() - yCorrection,
    )

    const result: ResultPoint[] = [
      topLeftCornerCenter,
      topRightCornerCenter,
      bottomRightCornerCenter,
      bottomLeftCornerCenter,
    ]
    return result
  }
